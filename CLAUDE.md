...
docs/              contributor docs; docs/mockup/ is the approved UI design — never edited or imported
```
...
| Any UI — site or desktop | `.claude/DESIGN.md` (the design system, always loaded) → `docs/mockup/README.md` → that app's `CLAUDE.md` → `packages/design-tokens/` |
...
5. **Anything a human and a program must both agree on is generated** — error codes, API fields, the subdomain rules, and the CLI flag reference (`schema/cli.json`, from `Args::command()`), which three files *claimed* was generated for weeks before it was (defect 38).
