...
5. **Anything a human and a program must both agree on is generated** — error codes, API fields, the subdomain rules. **The CLI flag reference is not yet**, though three files used to say it was: `crates/cli/src/args.rs` is still the only place flags are described (defect 38).
