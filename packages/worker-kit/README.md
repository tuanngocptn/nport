...
Consumers: `apps/api` (a node) gates `POST /v1/tunnels`; `apps/registry` gates `POST /v1/nodes`.
...
**No bindings, no `env`, no Hono.** That is the whole boundary, and it is what lets these tests run under plain vitest instead of `workerd`. Anything that needs a binding stays in the app that owns the binding — which is why the middleware did not move: `requestId`, `rateLimit`, `clientGate` and `requireBindings` are typed against `apps/api`'s own `Env`, and generalising them to share four small functions would trade clarity for reuse nobody asked for.
