...
src/views/tunnels.tsx                 the Tunnels screen; state lives in lib/, tested there
src/components/tunnel-card.tsx        one row, from the mockup's card
src/lib/tunnel-state.ts               the event stream reduced to rows — pure, and where the tests are
...
(src/views/          new-tunnel, inspector, history, settings — tunnels is written)
(src/components/     app-specific: RequestTable, JsonTree … — TunnelCard is written)
