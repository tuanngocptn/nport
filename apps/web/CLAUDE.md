...
4. **No raw hex colours in components.** Everything comes from `packages/design-tokens` via Tailwind utilities (ADR-0014). `.claude/DESIGN.md` is the design system behind those tokens and is always loaded. **Its §8 copy rules and this site's mockup disagree about the word "free"**, and the mockup wins here until someone decides otherwise: §8's stated reason is that the three-tunnel cap is a Cloudflare quota rather than a paywall, and the mockup's own copy already says exactly that ("No paid tier gating the URL you want"). Applying the rule literally to a sentence that already satisfies its reason is how a correction becomes a regression — `docs/ROADMAP.md`, defect 52.
