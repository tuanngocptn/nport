...
4. **No raw hex colours in components.** Everything comes from `packages/design-tokens` via Tailwind utilities (ADR-0014). `.claude/DESIGN.md` is the design system behind those tokens and is always loaded; **its §8 copy rules bind this app** — "free" is not a pricing claim to make anywhere, including marketing copy.
