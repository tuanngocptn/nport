 ...
 * `apps/node/.dev.vars` — a gitignored file that `wrangler dev` reads and `wrangler deploy` does not.
...
// Module scope, which `apps/node/CLAUDE.md` rule 10 forbids — and the reason it forbids it does not
