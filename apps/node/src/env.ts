 ...
 * to the log, which is where an operator is looking (`apps/node/CLAUDE.md` rule 8).
 */
...
/**
 * Bindings that are objects rather than strings: the Durable Object namespaces.
 *
 * `RATE_LIMITER` left with the rate limiter itself (ADR-0049) — it is the gateway's binding now, and
 * so is `IP_HASH_SECRET`. A Worker that no longer computes a source hash has no business holding the
 * key for one.
 *
 ...
 */
const REQUIRED_BINDINGS = ["SUBDOMAIN_LEASE", "REGISTRY", "SOURCE_QUOTA"] as const
