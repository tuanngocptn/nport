...
- **Test difficulty is pinned to 4 bits** in `vitest.config.ts`. A 20-bit solve is ~1M `crypto.subtle.digest` awaits inside `workerd` and times out — found the honest way, by writing a test that did.
