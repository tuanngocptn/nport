...
/**
 * Tests run in **real `workerd`**, for the same reason `apps/api`'s do: the directory is a Durable
    ...
    cloudflareTest({
      ...
      miniflare: {
        bindings: {
          ...
          //
          // **Deliberately not the same value `apps/api`'s tests use.** The two services sign
          ...
          // `apps/api`'s config warns: the pool reads that file alongside `wrangler.jsonc`.
        ...
        //
        // Same warning as `apps/api`: `isolatedStorage` does not exist in vitest-pool-workers 0.20
