...
8. **Answers to the open questions in §17 get written down** with a date, the moment the spike answers one. They are a deliverable.
...
- **Classify registration errors before retrying**: `EDUPCONN` means rotate the edge address (retrying the same one loops forever), while a cause containing `Unauthorized` is usually transient — a freshly created tunnel takes time to propagate, so retry before surfacing it.
