...
- `apps/desktop` is **not** in scope. Playwright cannot drive a Tauri WebView; that needs `tauri-driver` with WebdriverIO. The app has since gained a Vitest tier over its pure state and `cargo test` over its Rust half, but **neither renders anything** — `docs/TESTING.md`'s "Tauri WebView rendering is manual per platform" still stands, and by Phase 4's fourth screen it is the largest untested surface in the repository.
