...
**Every CLI it starts gets a throwaway `~/.nport`.** `config::path` reads `NPORT_HOME` before `HOME`, so without one every spawned binary loads the *developer's* config: a `lang` there turns assertions about English output red, and a `backend` there would point the whole run at somebody else's control plane and fail as though the tunnel were broken (`docs/ROADMAP.md`, defect 50). CI has no home directory to leak, so this passed there and failed only on a machine that had used the tool. **The rule is one home per run and no exceptions** — including the checks that write configs on purpose, which take their own inside it.
