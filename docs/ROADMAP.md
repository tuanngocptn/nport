...
**What building it needs decided first**, which is why this is recorded rather than rushed: `crates/cli` is a bin-only crate, so nothing can import `Args` to walk its clap definition. Three options, in preference order — (a) give `crates/cli` a `lib.rs` and let `crates/xtask` depend on `nport`, using `clap::CommandFactory`, which is how every Rust project generates completions and man pages and is exact rather than parsed; (b) keep a `FLAGS` table beside the clap struct with a test asserting the two agree, which avoids the new dependency edge but needs the table reachable from codegen anyway; (c) parse `--help` output, which is the string-matching this repository has an ADR against (ADR-0018). Option (a) also costs `cargo xtask codegen` a full CLI build, which the drift workflow already pays for. **The rendering half is the MDX docs item already listed above** — the generated artifact and the page it appears on are separate deliverables, and the artifact is the one three files were wrong about.
