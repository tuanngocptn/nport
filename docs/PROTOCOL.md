...
**Verified live 2026-08-03**, including a 30-minute sustained run: four connections came up 2/2 across `region1` and `region2`, landing in four distinct colos (`hkg01`, `hkg09`, `hkg11`, `hkg13`) — the balance is real, not nominal — and held **99.6–99.8% availability each** across five forced disconnects with zero dial or registration failures. Every loss rotated to the other region and re-registered in ~1 s.

One observation worth recording because it contradicts a reasonable assumption: **the edge tolerates the control stream closing right after registration.** `register_connection` drops its `RpcSystem` when it returns, which resets the control stream, and connections still served for 30 minutes. §12's graceful shutdown needs the stream held open to call `unregisterConnection`, but liveness does not depend on it. `crates/protocol`'s `AddressPool` implements the handout; two details of it are worth stating because they are not obvious from upstream:
...
2. **`Sec-Websocket-Key` does arrive in the metadata** like any ordinary header, and must be forwarded untouched — the origin derives `Sec-Websocket-Accept` from it and a substituted key makes the client reject the 101. Both halves of this are now pinned by `connect_request_websocket.bin`: the fixture test asserts the key is present *and* that `Connection`/`Upgrade` are absent, so if the edge ever starts sending them this section fails a test rather than quietly becoming wrong.
